...
def dashboard(request):
    ...
    total_musics = Music.objects.count()
    total_rates = Rate.objects.count()
    avg_rate = Rate.objects.aggregate(Avg('mark'))['mark__avg'] or 0
    
    # 音乐流派分布 (前10)
    genre_data = Tags.objects.annotate(count=Count('music')).order_by('-count')[:10]
    genre_labels = [g.name for g in genre_data]
    genre_values = [g.count for g in genre_data]
    
    # 热门歌手 TOP 10
    artist_data = Music.objects.values('artist').annotate(count=Count('id')).order_by('-count')[:10]
    artist_labels = [a['artist'] for a in artist_data]
    artist_values = [a['count'] for a in artist_data]
    
    # 评分分布统计 (1, 2, 3, 4, 5)
    rate_dist = Rate.objects.values('mark').annotate(count=Count('id')).order_by('mark')
    rate_labels = [str(r['mark']) + "星" for r in rate_dist]
    rate_values = [r['count'] for r in rate_dist]
...
def personal(request):
    ...
    # 【Apple Music 增强：计算用户音乐画像数据】
    from django.db.models import Count
    # 获取该用户评价过的所有歌曲及其标签
    user_rates = Rate.objects.filter(user_id=user_id).select_related('music')
    tag_counts = {}
    
    for rate in user_rates:
        for tag in rate.music.tags.all():
            tag_counts[tag.name] = tag_counts.get(tag.name, 0) + 1
            
    # 取前 6 个最喜欢的标签用于雷达图
    sorted_tags = sorted(tag_counts.items(), key=lambda x: x[1], reverse=True)[:6]
    chart_tags = [t[0] for t in sorted_tags]
    chart_values = [t[1] for t in sorted_tags]

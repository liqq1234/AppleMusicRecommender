    ...
    for i in range(user_numbers):
        ...
        try:
            ...
            for music_id in random_music_id():
                total_duration = random.randint(180, 300) # 3-5分钟
                skip_count = random.choice([0, 0, 0, 1, 2]) # 有一定概率跳过
                if skip_count == 0:
                    play_duration = random.randint(int(total_duration * 0.5), total_duration)
                else:
                    play_duration = random.randint(10, int(total_duration * 0.5))
                    
                Rate.objects.get_or_create(user=user, music_id=music_id, defaults={
                    "mark": random_mark(),
